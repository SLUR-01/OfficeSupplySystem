                    ...
                    @foreach ($requests as $request)
                        @if ($request->department === auth()->user()->department && $request->withdrawal_status !== 'Completed')
                            ...
                            <tr>
                                ...
                                <!-- Items Column -->
                                <td class="px-3 py-4 whitespace-nowrap text-sm text-dark">
                                    <ul class="space-y-1">
                                        @foreach ($request->items as $item)
                                            <li>
                                                <span class="font-bold">{{ $item->item_name }}</span>
                                                @if ($item->variant_value)
                                                    <div class="text-gray-500 text-xs">{{ $item->variant_value }}</div>
                                                @endif
                                            </li>
                                        @endforeach
                                    </ul>
                                ...
                                <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-700">
                                    <ul class="space-y-1">
                                        @foreach ($request->items as $item)
                                            <li class="font-medium">{{ $item->quantity }}</li>
                                        @endforeach
                                    </ul>
                                </td>
                                        ...
                                        <div class="w-full">
                                            <div class="w-full bg-gray-200 rounded-full h-2.5">
                                                <div class="{{ strpos($request->admin_status, 'Approved') !== false ? 'bg-teal-500' : 'bg-gray-300' }} h-2.5 rounded-full"
                                                    style="width: {{ strpos($request->admin_status, 'Approved') !== false ? '100%' : '0%' }}">
                                                </div>
                                            </div>
                                            <span
                                                class="block text-center mt-2 text-md font-medium {{ strpos($request->admin_status, 'Approved') !== false ? 'text-teal-600 font-semibold' : 'text-gray-500' }}">
                                ...
                                <td
                                    class="px-3 py-4 whitespace-nowrap text-sm  flex justify-center items-center space-x-2">
                                    <span
                                        class="inline-flex px-2 py-1 leading-5 font-semibold rounded-full
                                        @if ($request->withdrawal_status === 'Pending') bg-yellow-100 text-yellow-700
    ...
    <script>
        ...
        function filterRequests() {
            ...
            const searchValue = requestSearchInput.value.toLowerCase();
            const rows = document.querySelectorAll('#requestTbody tr');
            let visibleCount = 0;
        ...
        function showPrintModal(requestId) {
            ...
            const requesterName = document.querySelector('input[name="requester_name"]')?.value || reqData.requester_name ||
                'N/A';
            ...
            if (reqData.items && reqData.items.length > 0) {
                reqData.items.forEach(item => {
                    itemRows += `
                <tr>
                    <td class="border px-4 text-start py-2">${item.item_name}</td>
                    <td class="border px-4 text-start py-2">${item.quantity}</td>
                    <td class="border px-4 text-start py-2">${item.unit || 'N/A'}</td>
                </tr>
            `;
                });
            } else {
                itemRows = `
            <tr>
                <td colspan="3" class="border px-4 text-center py-2 text-gray-500">No items found</td>
            </tr>
        `;
            }
    ...
    <div class="overflow-y-auto">
        <div class="mb-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <p class="text-sm text-start"><span class="font-semibold">ID:</span> ${requestId}</p>
