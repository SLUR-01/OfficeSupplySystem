   ...
   <div class="gap-3 rounded-md gap-30 flex items-start mb-3">
       ...
       <div class="">
           <button id="processReturnBtn"
               class="flex px-2 py-2 bg-teal-500 text-white rounded hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
               disabled>
   ...
   <div class="bg-white rounded-lg overflow-hidden shadow-md">
       <div class="overflow-x-auto px-3">
           ...
           <table class="min-w-full bg-white divide-y divide-gray-200">
               <thead class="bg-white tracking-wide">
                   <tr>
                       ...
                       <th class="px-3 py-3 text-left text-sm text-gray-600 ">Completed at</th>
                       <th class="px-3 py-3 text-center text-sm text-gray-600 ">Withdrawal</th>
                       <th class="px-3 py-3 text-left text-sm text-gray-600 ">Withdrawn by </th>
                       <th class="px-3 text-left text-sm text-gray-600 ">Select</th>
                   </tr>
               </thead>
               <tbody id="withdrawalTbody" class="bg-white divide-y divide-gray-200">
                   ...
                   @foreach ($requests as $request)
                       @if (
                           $request->department === auth()->user()->department &&
                           ...
                           <tr>
                               ...
                               <td
                                   class="px-3 py-4 whitespace-nowrap text-sm font-medium flex justify-center items-center space-x-2">
                                   <span
                                       class="inline-flex px-2 py-1 leading-5 font-semibold rounded-full
                                            @if ($request->withdrawal_status === 'Processing') bg-yellow-100 text-yellow-800
                               ...
                               <td class="px-6 py-4 whitespace-nowrap text-sm text-dark capitalize">
                                   {{ $request->withdrawn_by }}</td>
                               <td class="px-3 py-4 whitespace-nowrap">
                                   @if (!$isReturned)
                                       <input type="radio" name="selectedRequest"
                                           class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                                           value="{{ $request->id }}" data-item-name="{{ $request->item_name }}"
           ...
           <div id="returnModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden z-50">
               ...
               <div id="rightModal"
                   class="fixed right-0 top-10 bottom-10 h-100 w-full max-w-md overflow-y-auto transform transition-transform duration-300 ease-in-out translate-x-0 bg-white shadow-xl z-50">
                   <div class="h-full overflow-y-auto p-4">
                       ...
                       <form action="{{ route('user.return.store') }}" method="POST" id="returnForm"
                           ...
                           <input type="hidden" id="requesterName" name="requester_name"
                           ...
                           <div id="otherConditionWrapper" class="mb-2 hidden">
                               <label for="other_condition"
                                   class="block text-sm font-medium text-gray-700 mb-1">Specify Other Condition</label>
                           ...
                           <!-- Image Proof -->
                           <div class="mb-4">
                               <label for="proof_image" class="block text-sm font-medium text-gray-700 mb-1">Upload
                                   Proof (Image)</label>
                               ...
                               <p class="mt-1 text-xs text-gray-500">Upload an image showing the item's condition (max
                                   5MB)</p>
                           </div>
                           ...
                           <div class="flex justify-end space-x-3">
                               ...
                               <button type="submit"
                                   class="px-4 py-2 bg-teal-600 text-white rounded-md text-sm font-medium hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                   Submit Return
                               </button>
                           </div>
                       </form>
       ...
       document.getElementById('returnForm').addEventListener('submit', function(e) {
           ...
           fetch(this.action, {
               ...
               .then(async response => {
                   ...
                   const errorMsg = doc.querySelector('.alert-danger, .error, [role="alert"]')?.textContent
                       .trim() || 'An error occurred while processing your request';
           ...
           if (selectedRadio) {
               ...
               if (selectedRadio.dataset.variantValue) {
                   itemNameDisplay.innerHTML =
                       `${selectedRadio.dataset.itemName} <span class="text-gray-500 text-sm">(${selectedRadio.dataset.variantValue})</span>`;
               } else {
