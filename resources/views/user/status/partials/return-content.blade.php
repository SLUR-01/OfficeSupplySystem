     ...
     <div class="overflow-x-auto px-3">
         ...
         <table class="min-w-full bg-white divide-y divide-gray-200">
             <thead class="bg-white tracking-wide">
                 <th class="py-3 text-left text-sm text-gray-600">Return ID</th>
                 <th class="px-3 py-3 text-left text-sm text-gray-600">Item ame</th>
                 <th class="px-3 py-3 text-left text-sm text-gray-600">Quantity returned</th>

                 <th class="px-3 py-3 text-left text-sm text-gray-600 ">Date returned</th>
                 <th class="px-3 py-3 text-left text-sm text-gray-600 ">Condition</th>
                 <th class="px-3 py-3 text-left text-sm text-gray-600 ">Status</th>
             ...
             <tbody id="returnTbody" class="bg-white divide-y divide-gray-200">
                 @foreach ($returns as $return)
                     @if ($return->replacement_status !== 'completed' && $return->return_status !== 'rejected')
                         <tr>
                             <td class="py-4 whitespace-nowrap text-sm font-semibold text-dark uppercase">RETURN#:
                                 {{ $return->id }}</td>
                             ...
                             <td class="px-3 py-3 whitespace-nowrap text-sm textdark capitalize">{{ $return->quantity }}
                             </td>
                             ...
                             <td class="px-3 py-3 whitespace-nowrap text-sm textdark capitalize">
                                 @switch($return->condition)
                                     @case('defective')
                                         <span
                                             class="px-2 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-800">{{ $return->condition }}
                                         </span>
                                     @break

                                     @case('damaged')
                                         <span
                                             class="px-2 py-1 text-xs font-semibold rounded-full bg-orange-100 text-orange-800">{{ $return->condition }}</span>
                                     @break

                                     @case('wrong_item')
                                         <span
                                             class="px-2 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-800">Wrong
                                             Item</span>
                                     @break
                             ...
                             <td class="px-3 py-3 whitespace-nowrap text-sm textdark capitalize">
                                 ...
                                 @switch($return->return_status)
                                     ...
                                     @case('rejected')
                                         <span
                                             class="px-2 py-1 text-xs font-semibold rounded-full bg-red-100 text-green-800">Rejected</span>
                                     @break
