...
@section('content')
    <div class="main-container bg-gray-100 p-[15px] h-100">
        ...
        @endif
        <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-start">
            <div>
        ...
        <div class="bg-white rounded-md overflow-hidden shadow-md">
            <div class="overflow-x-auto px-3 overflow-y-hidden">
                <table class="min-w-full bg-white divide-y divide-gray-200">
                    <thead class="bg-white tracking-wide font-medium">
                        <tr>
                            <th class="py-3 text-center text-md text-gray-600 ">User Id</th>
                            <th class="px-3 py-3 text-left text-md text-gray-600 ">Name</th>
                            <th class="px-3 py-3 text-left text-md text-gray-600 ">Email</th>
                            <th class="px-3 py-3 text-left text-md text-gray-600 ">Role</th>
                        ...
                        @foreach ($users as $user)
                            @if ($user->role != 'admin')
                                <tr>
                                    ...
                                    <td class="px-3 py-3 text-sm text-gray-800 text-left capitalize">{{ $user->role }}
                                    </td>
            ...
            <div id="editUserModal"
                class="fixed inset-0 z-50 flex mt-3 items-center justify-center hidden bg-black bg-opacity-50 overflow-y-auto">
                <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 overflow-y-auto">
                    ...
                    <div class="bg-gray-50 px-4 py-3 flex justify-end">
                        <button type="button" id="cancelEditUser"
                            class="mr-2 px-4 py-2 border border-gray-300 rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                            Cancel
                        </button>
                        <button type="submit" form="editUserForm"
                            class="px-4 py-2 border rounded-md shadow-sm text-white bg-teal-600 hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                            Update User
                        </button>
                    </div>
                </div>
                            ...
                            fetch(`/users/${userId}`, {
                                ...
                                .then(data => {
                                    if (data.success) {
                                        ...
                                        Swal.fire('Error', 'Could not delete user.',
                                            'error');
                                    }
                                });
                        }
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                Swal.fire({
                                    ...
                                    window.location
                                        .reload(); // Reload page to reflect new user in the list
                                });
                            } else {
                                Swal.fire({
                                    icon: 'error',
