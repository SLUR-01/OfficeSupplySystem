...
@section('content')
                        ...
                        @foreach ($requests as $request)
                            @if (in_array($request->withdrawal_status, ['Pending', 'Ready to Pick Up']) && $request->admin_status === 'Approved')
                                <tr class="hover:bg-gray-50 transition" data-request-id="{{ $request->id }}">
                                    ...
                                    <td class="px-3 text-md text-gray-800 max-w-[10px] truncate"
                                    ...
                                    <td class="px-3 py-4 text-sm relative text-center">
                                        <!-- Three Dots Button -->
                                        <button
                                            class="action-btn px-3 py-2 rounded-md text-white bg-teal-600 hover:bg-teal-700"
                                            onclick="toggleDropdown('dropdown-{{ $request->id }}')">
                                            Update
                                        </button>
                    ...
                    <div class="bg-white w-full max-w-md p-6 rounded-lg shadow-lg">
                        <div class="flex justify-between items-center pb-2">
                            ...
                            <button onclick="closeModal()"
                                class="text-gray-600 hover:text-gray-800 text-xl focus:outline-none">&times;</button>
                        </div>
                        <form id="withdraw-form" method="POST" action="">
                            ...
                            <div class="mt-4">
                                ...
                                <input type="text" name="withdrawn_by" id="withdrawn-by"
                            ...
                            <div class="mt-4">
                                ...
                                <input type="datetime-local" name="completed_at" id="completed-at"
                                    class="w-full border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500"
                                    required>
                            </div>
                            ...
                            <div class="mt-5 flex justify-end gap-2">
                                ...
                                <button type="submit"
                                    class="px-4 py-2 bg-teal-600 text-white rounded-md hover:bg-teal-700 focus:outline-none transition duration-200">
                                    Confirm
                                </button>
                            </div>
                        </form>
    ...
    <script>
                            ...
                            if (data.success) {
                                Swal.fire({
                                    ...
                                    const statusElement = document.querySelector(
                                        `tr[data-request-id="${requestId}"] .inline-flex`);
                                    statusElement.textContent = 'Ready to Pick Up';
                                    statusElement.className =
                                        `inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${data.status_class}`;
        ...
        function openWithdrawModal(requestId) {
            ...
            const withdrawnByInput = document.getElementById('withdrawn-by');
            withdrawnByInput.addEventListener('input', function() {
